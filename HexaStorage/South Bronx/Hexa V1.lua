...
titleLabel.BackgroundTransparency = 1; titleLabel.Text = "Hexa.lua"
...
discLbl2.BackgroundTransparency=1; discLbl2.Text="discord.gg/tkCSQAWMVR"
...
-- ========== PAGE: ESP + WHITELIST + VEHICLE FLY ==========
do
    ...
    etb.MouseButton1Click:Connect(function()
        ...
        if espEnabled then
            ...
            -- Sembunyikan semua drawing tapi jangan hapus cache
            -- (supaya langsung muncul lagi kalau di-enable)
...
do
    ...
    local infoCard2=makeCard(pageTP,120,22)
    makeLabel(infoCard2,"Kill → Respawn → TP ke tujuan",10,0,380,22,9,C.subtext)
    local tpLocs = {
        {name="Dealership",            color=Color3.fromRGB(0,110,220),  x=732.1171264648438,   y=3.3621320724487305,  z=406.0807189941406},
        {name="Jual/Beli Marshmellow", color=Color3.fromRGB(180,80,0),   x=510.9961853027344,   y=3.5872106552124023,  z=598.3929443359375},
        {name="Tier",                  color=Color3.fromRGB(120,0,200),  x=1094.7406005859375,  y=3.188796043395996,   z=158.09230041503906},
        {name="Casino",               color=Color3.fromRGB(180,120,0),  x=1154.863525390625,   y=4.289375305175781,   z=-46.8486328125},
        {name="Jual Casino",           color=Color3.fromRGB(160,100,0),  x=1017.5814819335938,  y=4.545021533966064,   z=-321.7923889160156},
        {name="GS Ujung",             color=Color3.fromRGB(80,80,80),   x=-464.5489501953125,  y=3.7371325492858887,  z=335.3158874511719},
        {name="GS Mid",               color=Color3.fromRGB(80,80,80),   x=218.74879455566406,  y=3.729842185974121,   z=-161.87036132812},
    }
    for i, loc in ipairs(tpLocs) do
        local locBtn=Instance.new("TextButton")
        locBtn.Size=UDim2.new(1,-20,0,36); locBtn.Position=UDim2.new(0,10,0,150+(i-1)*44)
        ...
        locBtn.BorderSizePixel=0; locBtn.Parent=pageTP; mkCorner(locBtn,5); mkStroke(locBtn,1,loc.color)
        ...
        local ci=i
        locBtn.MouseButton1Click:Connect(function() local l=tpLocs[ci]; tpTo(l.x,l.y,l.z) end)
    end

    local loopBase = 150 + #tpLocs*44 + 8
    ...
    local intCard2=makeCard(pageTP,loopBase+68,28)
    makeLabel(intCard2,"Interval: 30 detik  |  Target: Jual/Beli Marshmellow",10,0,430,28,10,C.subtext)
    ...
    pageTP.CanvasSize=UDim2.new(0,0,0,plrBase+180)
end
...
do
    ...
    makeLabel(infoCard,"Tidak perlu mati / respawn  |  Bisa dipakai saat naik motor",10,0,430,22,9,Color3.fromRGB(0,220,100))
    ...
    local vtpLocs = {
        ...
        {name="GS Ujung",             color=Color3.fromRGB(80,80,80),   x=-464.5489501953125,  y=3.7371325492858887,  z=335.3158874511719},
        {name="GS Mid",               color=Color3.fromRGB(80,80,80),   x=218.74879455566406,  y=3.729842185974121,   z=-161.87036132812},
        {name="Safe",                  color=Color3.fromRGB(0,220,100),  x=120.85433197021484,  y=4.297231197357178,   z=-587.6337280273438},
    }
    for i, loc in ipairs(vtpLocs) do
        ...
        btn.BorderSizePixel=0; btn.Parent=pageVehicleTP; mkCorner(btn,5); mkStroke(btn,1,loc.color)
        ...
        local ci=i
        btn.MouseButton1Click:Connect(function() local l=vtpLocs[ci]; tpVehicle(l.x,l.y,l.z) end)
    end
    ...
    for i, k in ipairs(kompors) do
        ...
        btn.Font=Enum.Font.GothamBold; btn.TextSize=12; btn.TextXAlignment=Enum.TextXAlignment.Left
        btn.BorderSizePixel=0; btn.Parent=pageVehicleTP; mkCorner(btn,5); mkStroke(btn,1,Color3.fromRGB(0,140,100))
        local pad=Instance.new("UIPadding",btn); pad.PaddingLeft=UDim.new(0,12)
        btn.MouseEnter:Connect(function() btn.BackgroundColor3=Color3.fromRGB(20,30,45) end)
        btn.MouseLeave:Connect(function() btn.BackgroundColor3=C.card end)
        local ci=i
        btn.MouseButton1Click:Connect(function() local kk=kompors[ci]; tpVehicle(kk.x,kk.y,kk.z) end)
    end
...
do
    ...
    local creditData={{role="Investor & Owner",name="Hexa.lua",color=Color3.fromRGB(255,215,0),initials="HI"},{role="The Biggest Femboy in world",name="Xorva",color=Color3.fromRGB(100,180,255),initials="V7"}}
    ...
    fLbl.Text='<font color="rgb(255,60,90)">discord.gg/tkCSQAWMVR</font>  -  <font color="rgb(100,120,140)">Thank you for using Hexa.lua</font>'
    ...
    local dLbl=makeLabel(dCard,"discord.gg/tkCSQAWMVR",0,0,0,30,11,C.subtext,Enum.Font.Gotham,Enum.TextXAlignment.Center); dLbl.Size=UDim2.new(1,0,1,0)
